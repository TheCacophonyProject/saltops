...
def update_salt_file(salt_path, repo_name, latest_release):
        ...
        if isinstance(value, dict) and "cacophony.pkg_installed_from_github" in value:
            for item in value["cacophony.pkg_installed_from_github"]:
                if (
                    ...
                    and item.get("version") != latest_release
                ):
    ...
    if updated:
        ...
        print(
            f"Salt file at {salt_path} is already up to date with release {latest_release}"
        )
    ...
    for sls_file in sls_files:
        ...
        for _, value in content.items():
            if (
                ...
                and "cacophony.pkg_installed_from_github" in value
            ):
...
def main():
    ...
    if not os.path.exists("github_token.txt"):
        print(
            "No github_token.txt file found. Please create one with your Github token. No permissions are needed for the github token it is just used to stop from getting rate limited."
        )
        return
    ...
    for repo in repo_info:
        ...
        try:
            ...
            if not is_up_to_date:
                print(
                    f"\tlatest release is not up to date with branch '{branch}'. Make a new release if needed."
                )
                continue
            ...
            if latest_release != repo_version:
                print(
                    f"\tlatest release is {latest_release} but {repo_version} is in the .sls file. Update the .sls file."
                )
            else:
                print(f"\tUp to date.")
